...
if ( ! function_exists('test_method'))
{
    ...
    function create_customer_dropdown($type,$flag=null) {
		...
		if($type == "dealer") {
			$sql = "SELECT id,name,companyname,name,dealercode FROM customer WHERE 
			is_deleted = 0
			and
			ctype=1 order by companyname";
			$ci=& get_instance();
			$ci->load->database(); 	
			$query = $ci->db->query($sql);
			$extra ="";
...
function create_pdf($content=null,$size ='A5') {
	if($content) {
...
function send_mail($to, $from,$subject="Cybera Email System",$content=null) 
{
	...
	return sendBulkEmail([$to], $from,$subject,$content);
...
function sendBulkEmail($to, $from,$subject="Cybera Email System",$content=null) 
{
	foreach($to as $sendTo) 
	{
		...
		$url = 'http://email.cyberaprint.com/';
		$url = urlencode($url);
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, urldecode($url));
		curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
...
function sendBulkEmail1($to, $from,$subject="Cybera Email System",$content=null) {
	$fromMail = getFromEmailId();
	$mail = new PHPMailer();
	$mail->Host     	= "smtp.gmail.com"; // SMTP server
	$mail->SMTPAuth    	= TRUE; // enable SMTP authentication
