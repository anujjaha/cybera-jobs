...
<div class="box">
	...
	<tbody>
		...
		foreach($masks as $mask) { 
			$maskTitle = str_replace(" ", "_2_", $mask['name']);
			$maskTitle = str_replace("(", "_4_", $maskTitle);
			$maskTitle = str_replace(")", "_5_", $maskTitle);
			?>
		<tr id="task_<?php echo $mask['id'];?>">
			<td><?php echo $sr;?></td>
			<td><?php echo $mask['name'];?></td>
			<td>
				<a href="<?= site_url();?>mask/in/<?= urlencode($maskTitle);?>">
					<?php echo $mask['totalInQty'];?>
				</a>
			<td>
				<a href="<?= site_url();?>user/mask/<?= $maskTitle;?>">
